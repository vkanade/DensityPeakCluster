...
class DistanceBuilder(object):
    ...
    def load_points(self, filename):
        ...
        Args:
            filename : file's name that contains all points. Format is a vector
                one line, each dimension value split by blank space
        '''
        with open(filename, 'r') as fp:
            for line in fp:
                self.vectors.append(np.array(list(map(float,
                                                      line.split('\t')))[:-1],
                                             dtype=np.float32))
    ...
    def build_distance_file_for_cluster(self, distance_obj, filename):
        ...
        Args:
            distance_obj : distance.Distance object for compute the distance of
                two point
        ...
        for i in range(len(self.vectors) - 1):
            for j in range(i, len(self.vectors)):
                fo.write(str(i + 1) + ' ' + str(j + 1) + ' ' +
                         str(distance_obj.distance(self.vectors[i],
                                                   self.vectors[j])) + '\n')
        fo.close()
# end DistanceBuilder
